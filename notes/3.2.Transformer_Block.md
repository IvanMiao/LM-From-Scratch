...
- **输出**: `q`, `k`, `v`，形状都还是 `(batch, seq_len, d_model)`。现在，每个词元都有了三个不同的身份表示。
...
- **输入**: `q`, `k`, `v`，形状为 `(batch, seq_len, d_model)`。
- **操作**: 使用 `einops` 进行维度重排。`'b s (h d) -> b h s d'` 的意思是：
	-   `b` 和 `s` 维度保持不变。
	-   将最后一个维度（`d_model`）拆分成 `h` (头的数量) 和 `d` (每个头的维度 `d_k`) 两部分。
	-   将 `h` 维度提到前面，形成一个新的维度。
- **输出**: `q`, `k`, `v`，形状变为 `(batch, num_heads, seq_len, d_k)`。这可以理解为我们为每个样本创建了 `num_heads` 个并行的“注意力宇宙”，每个宇宙的处理维度是 `d_k`。
...
- **操作**: 这是注意力的核心计算，在一个独立的函数中完成。它会执行 `softmax((q @ k.T) / sqrt(d_k) + mask) @ v`。
    ...
    -   `softmax` 将分数转为权重。
    -   `@ v` 用权重对Value进行加权求和。
- **输出**: `attention_output`，形状为 `(batch, num_heads, seq_len, d_k)`。这是每个头独立计算出的、融合了上下文信息的输出。
...
- **输入**: `attention_output`，形状为 `(batch, num_heads, seq_len, d_k)`。
- **操作1 (rearrange)**: 这是第2步的逆操作。它将 `num_heads` 和 `d_k` 两个维度重新**合并**成一个 `d_model` 维度。
- **中间输出**: `output`，形状恢复为 `(batch, seq_len, d_model)`。
- **操作2 (o_proj)**: 将合并后的张量喂给输出投影层 `self.o_proj`。
- **最终输出**: 形状仍为 `(batch, seq_len, d_model)`。这是经过所有头的信息融合、并由`o_proj`学习如何最佳组合后，得到的最终上下文表示。这个输出将作为下一个Transformer Block的输入。
